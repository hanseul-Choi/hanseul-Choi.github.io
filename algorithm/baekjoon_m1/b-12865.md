...
&nbsp;
# 문제 풀이
배낭 문제에는 두 가지의 문제 유형이 존재한다. 첫 번째는 쪼갤 수 있는 배낭문제와 0/1로 나눠지는 배낭문제로 나뉜다.
...
0/1 배낭 문제는 **Dynamic Programming방식**으로 풀어낼 수 있다. Dynamic Programming을 사용하는 이유는 이전 결과를 이용하여 값을 도출해내기 때문에 이전 결과 값을 저장하는 **Memoization방식**을 이용한다.

가장 중요한 포인트는 물건을 하나씩 넣으며 계산하는 것이다.
...
여기서 하나의 중요 포인트는 이전 최적 가치를 이용하여 가치표를 채워나간다는 것이다.
...
여기서 가장 중요한 **Dynamic Programming방식**으로 풀어야할 이유가 나온다.

3번의 경우 무게 1~6까지는 전부 위의 방식과 똑같이 동작한다. 무게 7을 보면 뜬금없이 14의 가치가 들어간 것을 확인할 수 있다.
이는 이전 최적 무게와 관련이 있다.
정확하게 따지면, 비교한 값은 13(이전 가치) < 8(현재 물건 가치) + 6(현재 물건 무게를 빼고도 남는 이전 최적 무게의 가치)이 된다.

즉, 4라는 무게는 총 7만큼 들어가지만 3이라는 공간이 더 들어갈 수 있다. 따라서, 이전 무게(2,3)의 최적 가치인 8과 현재 무게인 v를 더해주어 14라는 값을 도출하였고 이전 무게 7에서 최적이라고 계산했던 가치인 13과 비교하였을 때, 14가 크기 때문에 14를 넣을 수 있었다.
...
따라서, 무게 7에서의 최대 가치는 (4,7)인 **14**가 나온다.
