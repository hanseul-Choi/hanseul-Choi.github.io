...
토큰으로 작업이 실행됐다. 회의록 요청 성공률이 약 30% 수준까지 낮아졌다.
...
**결과** — 회의록 처리 성공률을 약 30%에서 99% 수준으로 개선했고, 수정 이후 7일 동안 동일한
리밸런싱 오류가 재발하지 않았다.
...
**결과** — 일간 504 오류를 100건 이상에서 약 2~5건 수준으로, 월간 약 6,000건 수준의 SSE
...
- 회의록 Kafka 처리 성공률 약 30% → 99%로 개선
- GPT·SSE 504 오류 일 100건 이상 → 약 2~5건으로 감소
