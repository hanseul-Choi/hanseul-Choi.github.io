...
복사 속도는 빨랐지만 모델을 NAS에서 직접 로딩할 때는 거의 2시간 이상 소요돼, 신규 모델 배포를
근무 외 시간에 진행해야 했다.
...
**결과** — 거의 2시간 이상 걸리던 NAS 직접 로딩을 배포 시점이 아닌 사전 다운로드 Job으로
옮겨, 실제 배포 시점에는 Node 로컬 디스크에서 즉시 로딩되도록 했다. 로딩 소요 시간 자체를
줄인 것이 아니라 근무 외 시간에만 가능했던 대용량 모델 배포를 업무 시간에도 진행할 수 있게
됐고, 다운로드와 서비스 기동 단계를 분리해 배포 실패 원인 파악도 쉬워졌다.
...
- 32B급 모델의 거의 2시간 이상 걸리던 NAS 직접 로딩 병목을 분석, GPU Node Local Cache로
  사전 다운로드해 근무 외 시간에만 가능했던 대용량 모델 배포를 업무 시간에도 가능하게 개선
